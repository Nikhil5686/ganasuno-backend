...
const STATE_PATH = path.join(__dirname, "playlist-state.json");
...
const PLAYLIST_TITLE = "GanaSuno — Bhojpuri Old";

const PLAYLIST_DESCRIPTION =
  "GanaSuno Bhojpuri Old catalog — curated music from the old Bhojpuri era.";
...
function channelQuality(channelName) {
  ...
  const match = POSITIVE_CHANNEL_WORDS.some((word) =>
...
function scoreResult(song, result, details) {
  ...
  const titleScore = tokenMatchScore(song.title, title) * 45;

  const artistScore = tokenMatchScore(song.artist, title) * 25;

  const channelScore = channelQuality(channel);

  let score = titleScore + artistScore + channelScore;

  /*
   * Views are only a supporting signal.
   */
  ...
  /*
   * Penalize extremely short videos.
   */
...
function loadTokens() {
  ...
  try {
    ...
    console.warn("⚠️ Could not read token.json.");
    return null;
  }
}
...
function loadPlaylistState() {
  ...
  try {
    ...
    console.warn("⚠️ Could not read playlist-state.json.");
...
function createInitialState(playlistId) {
  return {
    playlistId,
    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        res.end(`
          ...
          <html>
            <head>
              <title>GanaSuno Authorization</title>
            </head>
            ...
            ">
              ...
              <p>GanaSuno YouTube Playlist Manager is connected.</p>
...
async function searchSong(youtube, song) {
  const query = `${song.title} ${song.artist} Bhojpuri`;
  ...
  const scoredResults = results.map((result) => {
    ...
    const score = scoreResult(song, result, details);
...
async function main() {
  ...
  console.log("🎵 GanaSuno YouTube Playlist Manager");
  ...
  try {
    ...
    if (!state) {
      console.log("\n🆕 No existing playlist state found.");
      ...
      state = createInitialState(playlistId);

      savePlaylistState(state);

      console.log("\n💾 Playlist state saved.");
    } else {
      console.log("\n♻️ Existing playlist state found.");
    ...
    console.log("🎉 BOJPURI OLD CATALOG COMPLETE");
    ...
    console.log("\n💾 Final state saved to playlist-state.json");
  } catch (error) {
    console.error("\n❌ Error:");

    console.error(error.response?.data || error.message);

    console.log("\n💾 Your previous progress remains saved.");
  }
}
